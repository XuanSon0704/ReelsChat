...
The following ``stop-entities-detection-job`` example stops an in-progress, asynchronous entities detection job. If the current job state is ``IN_PROGRESS`` the job is marked for 
